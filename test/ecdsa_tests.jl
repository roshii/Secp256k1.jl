@testset "ECDSA" begin
    ...
    @testset "scep256k1" begin
        ...
        @testset "Order" begin
            point = secp256k1.N * G
            @test typeof(point) == Point{secp256k1.Infinity}
        end
        ...
        @testset "point2sec" begin
            ...
            point = coefficient * G
            @test point2sec(point, compressed=false) == hex2bytes(uncompressed)
            @test point2sec(point, compressed=true) == hex2bytes(compressed)
            coefficient = 123
            ...
            point = Point(parse(BigInt, "5689111130e588a12ecda87b2dc5585c6c6ba66a412fa0cce65bc", base =16),
