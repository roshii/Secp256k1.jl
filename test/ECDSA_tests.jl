@testset "ECDSA" begin
    ...
    @testset "DER Signature Parsing and Serialization" begin
        @testset "Parse" begin
            ...
            sig = ECDSA.Signature(big"0x1f62993ee03fca342fcb45929993fa6ee885e00ddad8de154f268d98f0839914",
                                  big"0x1e1ca12ad140c04e0e022c38f7ce31da426b8009d02832f0b44f39a6b178b7a1")
            @test ECDSA.parse(der) == sig
        end
        @testset "Serialize" begin
            ...
            for x in testcases
                sig = ECDSA.Signature(x[1], x[2])
                der = ECDSA.serialize(sig)
                sig2 = ECDSA.parse(der)
                @test sig2 == sig
            end
        end
    end
    ...
    @testset "Signature Verification" begin
        ...
        @test ECDSA.verify(point, z, ECDSA.Signature(r, s))
