...
function sec2point(sec_bin::AbstractArray{UInt8})
    ...
    is_even = sec_bin[1] == 2
    𝑥 = secp256k1.S256Element(bytes2int(sec_bin[2:end]))
    α = 𝑥^3 + secp256k1.S256Element(secp256k1.B)
    β = sqrt(α)
    if mod(β.𝑛, 2) == 0
        evenβ = β
        oddβ = S256Element(secp256k1.P - β.𝑛)
    else
        evenβ = S256Element(secp256k1.P - β.𝑛)
        oddβ = β
    end
    if is_even
